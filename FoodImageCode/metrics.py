...
from loss import cal_loss, cal_focal_loss, cal_l2_regularization
...
def cal_f1_score_acc(logits: torch.Tensor, label: torch.Tensor):
    '''
    ...
    tp, fp, fn, tn = cal_tp_fp_fn_tn(logits, label)
    ...
    precision_ma = torch.mean(torch.where(
        condition = (tp + fp) != 0,
        input = tp / (tp + fp),
        other = torch.zeros_like(tp)
    ))

    recall_ma = torch.mean(torch.where(
        condition = (tp + fn) != 0,
        input = tp / (tp + fn),
        other = torch.zeros_like(tp)
    ))
