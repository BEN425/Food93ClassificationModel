...
Separate the dataset in k folds and store in csv files for cross validatoin
'''
...
### Seperate images for k folds ###

folds = []

for category, image_lst in category_to_images.items():
    random.shuffle(image_lst)
    fold_size = ceil(len(image_lst) / K_FOLDS)
    
    for i in range(K_FOLDS) :
        folds.append(image_lst[i * fold_size : (i + 1) * fold_size])
