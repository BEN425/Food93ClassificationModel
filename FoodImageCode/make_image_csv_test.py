...
# console.print(list(images.items())[100000:100020])
# input()
...
# Case 1: Split multi-label images into training sets
# for category, image_lst in category_to_images.items():
#     # Get multi-label images
#     multi_images = list(filter(
#         lambda image: len(image["labels"]) > 1,
#         image_lst
#     ))
#     # Get single-label images
#     single_images = list(filter(
#         lambda image: len(image["labels"]) == 1,
#         image_lst
#     ))
    
#     # If multi-label images are more than 80%, random split
#     # Else, split multi-label images into training set
#     if len(multi_images) > len(image_lst) * 0.8 :
...
#         print(category)
#     else :
#         train_set.extend(multi_images)
#         remained = int(len(image_lst) * 0.8 - len(multi_images))
        
#         random.shuffle(single_images)
#         train_size = int(remained)
#         val_size = (len(single_images) - remained) // 2
        
#         train_set.extend(single_images[:train_size])
#         valid_set.extend(single_images[train_size : train_size+val_size])
...
for category, image_lst in category_to_images.items():
    # Find failed images
    failed_imgs = [img for img in image_lst if img["path"] in failed]
    remained_images = [img for img in image_lst if img not in failed_imgs]
    
    # If multi-label images are more than 80%, random split
    # Else, split failed images into training set
    if len(failed_imgs) > len(image_lst) * 0.8 :
        random.shuffle(image_lst)
        train_size = int(len(image_lst) * 0.8)
        valid_size = int(len(image_lst) * 0.1)
        train_set.extend(image_lst[:train_size])
        ...
        random.shuffle(remained_images)
        
        train_size = int(len(image_lst) * 0.8) - len(failed_imgs)
        valid_size = int(len(image_lst) * 0.1)
        
        train_set.extend(remained_images[:train_size])
        valid_set.extend(remained_images[train_size : train_size+valid_size])
        test_set.extend(remained_images[train_size+valid_size:])
